...
import { Shield, Sword, Lock, Plus, Check, LogOut, Loader2, Play, Eye, ChevronDown, Castle, Link as LinkIcon, RefreshCw, FileJson, Code } from "lucide-react";
...
// --- COMPLETE CLASH OF CLANS ID MAP (For Auto-Links) ---
...
export default function AdminPanel() {
  ...
  useEffect(() => {
    if (stratForm.armyComp.length === 0) {
        if(stratForm.armyLink.includes('link.clashofclans.com')) {
    ...
    if(stratForm.armyLink !== link) {
        setStratForm(prev => ({ ...prev, armyLink: link }));
    }
  }, [stratForm.armyComp]);
  ...
  const handleBulkSubmit = async () => {
      if (!jsonInput) return alert("Please paste valid JSON data.");
      setLoading(true);
          ...
          if (activeTab === 'layout') {
              for (const item of dataArray) {
                  const res = await addLayout({
                      title: item.title,
                      townHall: item.townHall || item.town_hall,
                      type: item.type,
                      imageUrl: item.imageUrl || item.image_url,
                      copyLink: item.copyLink || item.copy_link
                  });
                  if (res.success) successCount++; else skipCount++;
              }
          } else {
              for (const item of dataArray) {
                   const res = await addStrategy({
                       ...
                       townHalls: item.townHalls || [item.townHall],
                       difficulty: item.difficulty,
                       videoUrl: item.videoUrl || item.video_url,
                       armyComp: item.armyComp || item.army_comp,
                       armyLink: item.armyLink || item.army_link
                   });
                   if (res.success) successCount++; else skipCount++;
              }
          }

          let msg = `✅ Imported ${successCount} items.`;
          if (skipCount > 0) msg += ` (${skipCount} duplicates skipped)`;
          setMessage(msg);
          if (successCount > 0) setJsonInput('');
  ...
  const handleLayoutSubmit = async () => {
    ...
    setLoading(true);
    try {
        const res = await addLayout(layoutForm);
        if(!res.success) {
            alert(res.error);
        } else {
  ...
  const handleStratSubmit = async () => {
    ...
    setLoading(true);
    try {
        const res = await addStrategy({ ...stratForm, townHalls: [stratForm.townHall] });
        if(!res.success) {
            alert(res.error);
        } else {
    ...
    <div className="relative w-full" ref={thDropdownRef}>
        ...
        {isTHDropdownOpen && (
            <div className="absolute top-full mt-2 left-0 w-full max-h-60 overflow-y-auto bg-[#1f2937] border border-white/10 rounded-xl shadow-2xl z-50 custom-scrollbar animate-in fade-in zoom-in-95">
                {TOWN_HALLS.map(th => (
                    <button key={th} onClick={() => { onChange(th); setIsTHDropdownOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-2 hover:bg-white/5 transition-colors text-left border-b border-white/5 last:border-0 ${value === th ? 'bg-skin-primary/10' : ''}`}>
                        <div className="relative w-8 h-8 flex items-center justify-center bg-[#0c1015] rounded border border-white/10 overflow-hidden">
  ...
  return (
    <div className="max-w-6xl mx-auto py-10 px-4 min-h-screen pb-24">
       ...
       <div className="flex flex-col md:flex-row gap-4 mb-8">
          ...
          <button onClick={() => setJsonMode(!jsonMode)} className={`flex items-center justify-center gap-2 px-6 py-4 rounded-xl font-bold border ${jsonMode ? 'bg-white/10 border-skin-primary text-skin-primary' : 'bg-[#1f2937] border-white/5 text-skin-muted hover:text-white'}`}>
       ...
       {jsonMode ? (
         <div className="bg-[#1f2937] p-6 rounded-2xl border border-white/5 space-y-5 shadow-xl animate-in fade-in">
             <div className="flex justify-between items-center border-b border-white/5 pb-4">
                 <h3 className="text-sm font-bold text-white uppercase flex items-center gap-2"><FileJson size={16} className="text-skin-primary"/> Bulk Import ({activeTab === 'layout' ? 'Layouts' : 'Strategies'})</h3>
                 <span className="text-[10px] text-skin-muted bg-black/20 px-2 py-1 rounded">JSON Array Supported</span>
             </div>
             <textarea 
                value={jsonInput}
                onChange={(e) => setJsonInput(e.target.value)}
                placeholder={activeTab === 'layout' ? '[ { "title": "Base 1", ... } ]' : '[ { "title": "Strat 1", ... } ]'}
                className="w-full h-96 bg-[#0c1015] p-4 rounded-xl text-xs font-mono text-green-400 border border-white/10 outline-none focus:border-skin-primary resize-y"
             />
             <button onClick={handleBulkSubmit} disabled={loading} className="admin-btn-primary">{loading ? <Loader2 size={20} className="animate-spin"/> : <><Plus size={20}/> Import Data</>}</button>
         </div>
       ) : (
         /* === VISUAL MODE === */
         <>
              ...
              <div className="flex flex-col gap-8 animate-in fade-in">
                ...
                <div className="space-y-2">
                    ...
                    <div className="group bg-[#1a232e] border border-white/5 rounded-2xl overflow-hidden flex flex-col shadow-2xl max-w-sm mx-auto">
                        ...
                        <div className="p-4 space-y-3"><h3 className="font-bold text-white text-lg leading-tight">{layoutForm.title || "Base Title Preview"}</h3></div>
                    </div>
                </div>

                <button onClick={handleLayoutSubmit} disabled={loading} className="admin-btn-primary mt-4">{loading ? <Loader2 size={20} className="animate-spin"/> : <><Plus size={20}/> Save Layout</>}</button>
              </div>
           )}
           ...
           {activeTab === 'strategy' && (
              <div className="flex flex-col gap-8 animate-in fade-in">
                <div className="bg-[#1f2937] p-6 rounded-2xl border border-white/5 space-y-5 shadow-xl">
                    ...
                    <UnitSelector army={stratForm.armyComp} setArmy={(army) => setStratForm({...stratForm, armyComp: army})} filterTH={stratForm.townHall} />
                    ...
                    <div className="relative">
                        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><LinkIcon size={14} className="text-skin-muted"/></div>
                        <input value={stratForm.armyLink} readOnly placeholder="Link auto-generated..." className="admin-input pl-10 text-xs text-skin-muted cursor-not-allowed opacity-80"/>
                    </div>
                </div>
                ...
                <div className="space-y-2">
                    <div className="bg-[#1a232e] border border-white/5 rounded-2xl p-4 shadow-xl">
                        <div className="flex justify-between items-start mb-2"><h3 className="font-bold text-white text-lg">{stratForm.title || "Strategy Title"}</h3></div>
