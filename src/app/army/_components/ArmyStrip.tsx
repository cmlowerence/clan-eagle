...
export default function ArmyStrip({ army, updateUnit }: Props) {
  ...
  return (
    <div className="mb-6 sticky top-[120px] z-30 bg-skin-bg/95 backdrop-blur py-3 border-b border-skin-primary/5 animate-in slide-in-from-top-2">
      <div className="text-[10px] uppercase font-bold text-skin-muted mb-2 px-1">Current Army</div>
      
      <div className="flex gap-4 overflow-x-auto pb-4 pt-2 no-scrollbar px-2 min-h-[90px] items-start">
        {sortedUnits.map(([name, count]) => (
          <div key={name} className="relative group w-[64px] shrink-0">
            
            {/* Card Container (Inner overflow hidden only) */}
            ...
            {/* MINUS BUTTON - FIX: Darker color, positioned outside overflow */}
            <button 
