...
export default function ClanMembers({ members }: { members: ClanMember[] }) {
  // Safety Check: Prevent crashes if members are undefined
  if (!members || members.length === 0) {
    return <div className="p-8 text-center text-skin-muted opacity-50">No member information available.</div>;
  }
      ...
      {members.map((member, i) => {
        ...
        return (
          <Link
            ...
            className="group relative bg-[#2a3a4b] border border-[#3a4a5b] hover:border-[#5c9dd1] rounded-lg h-auto py-2 md:py-0 md:h-[70px] flex flex-row items-center overflow-hidden transition-all hover:-translate-y-0.5 shadow-md"
          >
            {/* Rank Column */}
              ...
              <div className="w-10 md:w-16 h-full flex items-center justify-center shrink-0">
                {member.leagueTier?.iconUrls?.small ? (
                  ...
                  <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center">
                    <Trophy size={14} className="text-skin-muted" />
                  </div>
                )}
              </div>
            </div>
