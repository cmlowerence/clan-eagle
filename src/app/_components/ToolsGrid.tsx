...
import { Sword, LayoutGrid, Crown, Globe, Scale } from "lucide-react";
...
export default function ToolsGrid() {
  const tools = [
    { 
        ...
        hover: "hover:bg-skin-primary/10 hover:border-skin-primary/30" 
    },
    { 
        href: "/layouts", 
        label: "Base Layouts", 
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 w-full animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-200">
      {tools.map((tool) => (
        <Link
          key={tool.href}
          href={tool.href}
        ...
        >
          ...
          <span className="text-xs font-bold uppercase text-skin-text whitespace-nowrap">{tool.label}</span>
        </Link>
      ))}
    </div>
  );
