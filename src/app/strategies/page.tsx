...
import { Play, Swords, Zap, Filter, ShieldAlert, ArrowLeft, Crown, Video } from "lucide-react";
                      ...
                      <div className="flex flex-col gap-4">
                         <div>
                            <div className="flex items-center gap-3 mb-2">
                                {strat.town_halls.map(th => (<span key={th} className="text-[10px] font-bold text-black bg-skin-primary px-2 py-0.5 rounded shadow-sm shadow-skin-primary/20">TH{th}</span>))}
                        ...
                        <div className="relative aspect-video w-full rounded-2xl overflow-hidden bg-black border border-white/10 shadow-2xl group-hover:border-white/20 transition-all mt-4">
                            {isPlaying ? (
                                ...
                                <button onClick={() => setPlayingVideo(strat.id)} className="group/video w-full h-full relative flex items-center justify-center">
                                    <img src={`https://img.youtube.com/vi/${videoId}/maxresdefault.jpg`} className="absolute inset-0 w-full h-full object-cover opacity-80 group-hover/video:opacity-100 transition-opacity duration-500" alt="Video Thumbnail"/>
                                    ...
                                    <div className="relative flex flex-col items-center gap-3 transition-transform duration-300 group-hover/video:scale-105">
                                        <div className="w-16 h-12 bg-[#FF0000] rounded-xl flex items-center justify-center shadow-[0_10px_20px_rgba(0,0,0,0.5)]">
                          ...
                          {troops.length > 0 && (
                             <div>
                                <div className="text-[10px] uppercase font-bold text-skin-muted mb-4 flex items-center gap-2 border-b border-white/5 pb-2">
                                   <Swords size={12} className="text-skin-primary"/> Army Composition
                                </div>
