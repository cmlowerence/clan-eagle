...
import { getUnitIconPath, UNIT_CATEGORIES, getUnitCategory, getCategoryIcon } from "@/lib/unitHelpers";
...
// ... [Keep Interfaces Unit, PlayerData] ...
interface Unit {
  name: string;
  level: number;
  maxLevel: number;
...
export default function PlayerPage({ params }: { params: { tag: string } }) {
  ...
  // Eagle Eye: Save to history on load
  useEffect(() => {
    if (player) {
  ...
  const UnitCard = ({ unit, type }: { unit: Unit, type: string }) => {
    ...
    return (
      <div className={`relative flex items-center gap-3 p-2 rounded-lg border transition-all overflow-hidden group ${isMax ? 'bg-gradient-to-r from-skin-primary/10 to-transparent border-skin-primary shadow-[0_0_15px_-5px_var(--color-primary)]' : 'bg-skin-surface border-skin-primary/10 hover:border-skin-primary/30'}`}>
         {isMax && <div className="absolute inset-0 bg-skin-primary/5 animate-pulse pointer-events-none"></div>}
         <div className="w-12 h-12 relative shrink-0">
            <img src={iconPath} onError={(e) => { e.currentTarget.style.display = 'none'; e.currentTarget.nextElementSibling?.classList.remove('hidden'); }} alt={unit.name} className={`object-contain w-full h-full drop-shadow-md transition-transform group-hover:scale-110 ${isMax ? 'brightness-110' : ''}`}/>
